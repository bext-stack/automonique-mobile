...
| Check rerun                                                                                        | Approved for an exact server-issued capability preview                                         | Requires separate capability-read, rerun, and receipt grants; exact workspace/review/check revisions and CI authority; an in-memory single-use confirmation; and durable lookup custody before submit. |
...
- Check-rerun confirmation digests are held only in a frozen, in-memory,
  single-use preview. The durable review receipt handle hashes the digest with
  the exact action coordinates, so restart recovery can only query the receipt
  by idempotency key and cannot replay the provider action.
