...
export function admitReviewV2ReceiptHandle(
  ...
  if (
    ...
    candidate.schema !== REVIEW_V2_RECEIPT_HANDLE_SCHEMA ||
    !['user_workspace', 'attempt_workspace'].includes(
      String(candidate.workspace_kind),
    ) ||
    candidate.authority_kind !== 'review' ||
    !['add_comment', 'approve_review'].includes(String(candidate.action_kind))
  ) {
    throw new Error('review_receipt_handle_invalid');
  }
...
export function createReviewV2ReceiptStore(
  ...
  const coordinates = async () => {
    ...
    return { key: storageKey(family), authorization };
  };
  return {
    async list() {
      const { key } = await coordinates();
      return locked(
        key,
        async () => admitIndex(await AsyncStorage.getItem(key)).handles,
      );
    },
    async put(value) {
      const handle = admitReviewV2ReceiptHandle(value);
      const { key, authorization } = await coordinates();
